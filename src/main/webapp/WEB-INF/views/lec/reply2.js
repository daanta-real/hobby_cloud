	...
	$.ajax({
		...
		success : function(resp){
			...
			for(var i=0; i<resp.length; i++){
				...
				template = template.replace("{{lecReplyIdx}}",resp[i].gatherReplyIdx);
				template = template.replace("{{memberNick}}",resp[i].memberNick);
				template = template.replace("{{lecReplyDetail}}",resp[i].gatherReplyDetail);
				template = template.replace("{{lecReplyRegister}}",resp[i].gatherReplyDate);
