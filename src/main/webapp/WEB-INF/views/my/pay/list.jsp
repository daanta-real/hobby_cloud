...
<c:set var="isAdmin" value="${memberGrade eq '관리자'}"></c:set>
<!DOCTYPE HTML>
<HTML LANG="ko">
...
value="${param.paidRegistered_end}"--%>
		...
		<div class="row p-sm-2 mx-1 mb-5">
			<div class="container">
				<form name="searchForm" method="get" class="row d-flex justify-content-center">
					<c:if test="${isAdmin}">
					...
					<table class="table table-striped table-hover table-bordered table-sm table-responsive m-0">
						...
						<tbody>
							<c:forEach var="paidVO" items="${paidList}">
								<tr class="cursor-pointer" onclick='location.href="${root}/my/pay/detail/${paidVO.paidIdx}"'>
									...
									<td class="text-center align-middle text-nowrap text-${paidVO.paidStatus ? 'success' : 'danger'}">
										<c:choose>
											<c:when test="${paidVO.paidStatus}">
												결제 완료&nbsp;<span onclick="
													stopEvent();
													if(confirm('정말로 결제를 취소하시겠습니까?')) location.href='${root}/my/pay/cancel/${paidVO.paidIdx}';"
													class="btn btn-warning btn-sm text-danger">결제 취소</span>
											</c:when>
											<c:otherwise>
												결제 취소됨
											</c:otherwise>
